...
describe('CalendarWidgetComponent', () => {
  ...
  const tomorrow = new Date("2023-12-25")
  ...
  it('should have empty lists at creation', () => {
    expect(component.todayEvents).withContext('with today events empty at first').toEqual([]);
    expect(component.futureEvents).withContext('with future events empty at first').toEqual([]);
  });
  ...
  it('should sort events starting today', () => {
    ...
    expect(component.todayEvents).withContext("with event 1 in today's list").toContain(events[0])
    expect(component.futureEvents).withContext("with event 2 in future list").toContain(events[1])
  });
